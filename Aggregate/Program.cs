...
namespace Aggregate
{
	class Program
	{
		public static void Main()
		{
			...
			System.IO.File.WriteAllLines(@"output.txt", header.Concat(rows
										 .Select(x => x.Split('\t'))
										 .Select(x => new Tuple<DateTime, float[]>(round(DateTime.Parse(x[0])), x.Skip(1).Select(y => float.Parse(y)).ToArray()))
										 .GroupBy(x => x.Item1, x => x.Item2)
										 .Select(x => x.Key.ToString("HH:mm") + "\t" + string.Join("\t", Scale(Transpose(x.ToArray())).Select(y => y.Max())))));
		}

		static float[][] Scale(float[][] input)
		{
